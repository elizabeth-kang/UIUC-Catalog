...
public final class Client {
  ...
  /**
   * Posts a rating for the summary.
   *
   * @param summary the summary to retrieve
   * @param rating the rating to retrieve
  ...
  public void postRating(
    ...
    String url = CourseableApplication.SERVER_URL + "rating/" + summary.getYear() + "/"
            + summary.getSemester() + "/" + summary.getDepartment()
            + "/" + summary.getNumber() + "?client=" + rating.getId();
    StringRequest ratingRequest =
            ...
            public byte[] getBody() throws AuthFailureError {
              String ratingAsString = null;
                try {
                  ratingAsString = objectMapper.writeValueAsString(rating);
                } catch (JsonProcessingException e) {
                  e.printStackTrace();
                }
              return ratingAsString.getBytes();
            }
            };
  ...
  /**
   * Retrieve rating for the summary.
   *
   * @param summary summary to retrieve
   * @param clientID clientID to retrieve
   * @param callbacks the callback that will receive the result
   */
  public void getRating(
    ...
    String url = CourseableApplication.SERVER_URL + "rating/" + summary.getYear() + "/"
            + summary.getSemester() + "/" + summary.getDepartment()
            + "/" + summary.getNumber() + "?client=" + clientID;
    StringRequest ratingRequest =
            new StringRequest(
                    Request.Method.GET,
                    url,
  ...
  /**
   * Start an API client for use in testing.
   *
   * @return the course API client for testing
   */
